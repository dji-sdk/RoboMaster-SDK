...
.. function:: sensor_adapter_ctrl.get_sensor_adapter_adc(board_id, port_num)
    ...
    :描述: 獲取傳感器轉接模塊相應端口模擬引腳的 ADC 值

    :param int board_id: 傳感器轉接模塊編號，範圍為[1:6]
    :param uint8 port_num: 傳感器轉接模塊上的端口號，範圍為[1:2]
    :param bool wait_for_complete: 是否等待執行完成，默認為 True

    :return: 傳感器轉接模塊相應端口模擬引腳的 ADC 值，範圍為[0:1023]
    :rtype: uint16
...
.. function:: sensor_adapter_ctrl.get_sensor_adapter_pulse_period(board_id, port_num)
    ...
    :描述: 獲取傳感器轉接模塊相應端口引腳的脈衝持續時間

    :param int board_id: 傳感器轉接模塊編號，範圍為[1:6]
    :param uint8 port_num: 傳感器轉接模塊上的端口號，範圍為[1:2]

    :return: 傳感器轉接模塊相應端口引腳的脈衝持續時間，精確度為 1 ms
    :rtype: uint32
...
.. function:: sensor_adapter_ctrl.cond_wait(rm_define.cond_sensor_adapter[board_id]_port[port_id]_[judge_type]_event)
    ...
    :描述: 等待傳感器轉接模塊相應端口引腳脈衝為（高/低/跳變）時，執行下一條指令
...
.. function:: sensor_adapter_ctrl.check_condition(rm_define.cond_sensor_adapter[board_id]_port[port_id]_[judge_type]_event)
    ...
    :描述: 判斷傳感器轉接模塊相應端口引腳脈衝是否為（高/低/跳變）
...
.. hint:: 模塊說明請參考 :doc:`傳感器轉接模塊 <../extension_module/sensor_adapter>`